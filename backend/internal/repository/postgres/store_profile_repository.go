...
func (r *StoreProfileRepository) Upsert(ctx context.Context, p *domain.StoreProfile) error {
	const q = `
		INSERT INTO store_profiles (user_id, owner_name, business_name, business_category, phone, address, updated_at)
	...
	err := r.pool.QueryRow(ctx, q,
		...
		p.BusinessCategory, p.Phone, p.Address, p.UpdatedAt,
...
func (r *StoreProfileRepository) ListAll(ctx context.Context) ([]*domain.StoreProfile, error) {
	const q = `SELECT id, user_id, owner_name, business_name, business_category, phone, address, created_at, updated_at FROM store_profiles ORDER BY created_at DESC`
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&p.BusinessCategory, &p.Phone, &p.Address, &p.CreatedAt, &p.UpdatedAt,
		); err != nil {
			return nil, err
		}
...
func (r *StoreProfileRepository) FindByUserID(ctx context.Context, userID string) (*domain.StoreProfile, error) {
	const q = `SELECT id, user_id, owner_name, business_name, business_category, phone, address, created_at, updated_at FROM store_profiles WHERE user_id = $1`
	p := &domain.StoreProfile{}
