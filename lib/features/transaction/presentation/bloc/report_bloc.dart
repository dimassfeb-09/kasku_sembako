...
class ReportBloc extends Bloc<ReportEvent, ReportState> {
  ...
  Future<void> _onVoidTransaction(VoidTransactionEvent event, Emitter<ReportState> emit) async {
    ...
    await voidResult.fold(
      ...
      (_) async {
        ...
        final reloadResult = await getTransactionsUseCase(currentState.startDate, currentState.endDate);
        reloadResult.fold(
          (failure) => emit(ReportError(failure.message)),
          (data) {
            emit(ReportLoaded(data, currentState.startDate, currentState.endDate));
          },
        );
      },
    );
  ...
  Future<void> _onExportPdf(ExportPdfEvent event, Emitter<ReportState> emit) async {
    ...
    try {
      await exportService.exportToPdf(currentState.transactions, currentState.startDate, currentState.endDate);
  ...
  Future<void> _onExportExcel(ExportExcelEvent event, Emitter<ReportState> emit) async {
    if (state is! ReportLoaded) return;
    final currentState = state as ReportLoaded;
    if (currentState.transactions.isEmpty) return;
    ...
    emit(ReportExporting());
    try {
      await exportService.exportToExcel(currentState.transactions, currentState.startDate, currentState.endDate);
